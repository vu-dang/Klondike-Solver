...
//
//In both cases every surviving branch is still searched exhaustively, so the result is
...
int main(int argc, char * argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		if (_stricmp(argv[i], "-game") == 0 || _stricmp(argv[i], "/game") == 0 || _stricmp(argv[i], "-g") == 0 || _stricmp(argv[i], "/g") == 0) {
			...
			cout << "SimpleSolver [/G #] [/DC #] [/S #] [/SIMPLER] [/MVS] [/R] [#]\n\n";
	...
	const char * method = simpler ? "simpler" : "simple";
	cout << "FC Game #" << gameNumber << " (draw " << drawCount << ", " << method << " method)\n";
	cout << s.GameDiagram() << "\n\n";

	clock_t start = clock();
	SolveResult result = simpler ? s.SolveSimpler(maxStates) : s.SolveSimple(maxStates);
